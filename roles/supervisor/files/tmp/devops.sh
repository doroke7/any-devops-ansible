...
sudo supervisorctl -c /etc/supervisord.conf restart lottery-service_resource_backend-lumen:

# 重启 lottery-crawler_all_backend-sracpy GROUP 所有 命令 【要加 冒号!!!!!】【要加 冒号!!!!!】【要加 冒号!!!!!】【要加 冒号!!!!!】 
sudo supervisorctl -c /etc/supervisord.conf restart lottery-crawler_all_backend-sracpy:



# 关闭 lottery-service_resource_backend-lumen php73_artisan_swoole_http_start
sudo supervisorctl -c /etc/supervisord.conf stop lottery-service_resource_backend-lumen:php73_artisan_swoole_http_start

# 启动 lottery-service_resource_backend-lumen php73_artisan_swoole_http_start
sudo supervisorctl -c /etc/supervisord.conf start lottery-service_resource_backend-lumen:php73_artisan_swoole_http_start
